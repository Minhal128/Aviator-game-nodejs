...
assert(is_file(dirname(base_path()) . '/Chicken-Road/.htaccess'), 'chicken htaccess');
assert(str_contains(file_get_contents(dirname(base_path()) . '/user/canvas.js'), '/images/canvas/plane-2.svg'), 'aviator plane asset exists in repo');
assert(str_contains(file_get_contents(dirname(base_path()) . '/Chicken-Road/Main/js/audio.js'), "preload = 'none'"), 'chicken audio not eager');
assert(str_contains(file_get_contents(base_path('resources/views/crash.blade.php')), '$sockLive'), 'crash skips localhost socket.io');
